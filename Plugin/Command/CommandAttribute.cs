...
public abstract class CommandAttribute : FunctionAttributeBase
{
    ...
    protected void AddColoredCommand(SeStringBuilder builder, string command)
    {
        builder.AddUiForeground(command, 37);
    }
    protected void AddIndentSpacing(SeStringBuilder builder)
    {
        builder.AddText(new string(' ', 8));
    }
    protected void AddHelpText(SeStringBuilder builder, string helpText)
    {
        builder.AddText($" - {helpText}");
    }
